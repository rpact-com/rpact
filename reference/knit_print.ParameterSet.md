...
The function `knit_print.ParameterSet` is the default printing function
for rpact result objects in knitr. The chunk option `render` uses this
function by default. To fall back to the normal printing behavior set
the chunk option `render = normal_print`. For more information see
