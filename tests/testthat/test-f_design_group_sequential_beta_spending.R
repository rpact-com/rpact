...
## |  Creation date: 30 March 2022, 14:01:33
## |  File version: $Revision: 6288 $
## |  Last changed: $Date: 2022-06-10 13:23:18 +0200 (Fri, 10 Jun 2022) $
## |  Last changed by: $Author: pahlke $
## |

context("Testing the Group Sequential Design Functionality with Two-Sided Beta Spending")

test_that("'getDesignGroupSequential' with two-sided beta spending, kMax = 4, non-binding futility, KD beta spending, gammaB = 1.5", {
    suppressWarnings(
...
test_that("'getDesignGroupSequential' with two-sided beta spending, kMax = 4, binding futility, KD beta spending, gammaB = 1.5", {
    .skipTestIfDisabled()
...
test_that("'getDesignGroupSequential' with two-sided beta spending, kMax = 4, non-binding futility, user defined beta spending (1)", {
    .skipTestIfDisabled()
    ...
    suppressWarnings(
        x <- getDesignGroupSequential(
            ...
            typeBetaSpending = "bsUser", userBetaSpending = c(0.15, 0.25, 0.8, 1) * 0.07, bindingFutility = FALSE
        )
    )
...
test_that("'getDesignGroupSequential' with two-sided beta spending, kMax = 4, non-binding futility, user defined beta spending (2)", {
    .skipTestIfDisabled()
    ...
    suppressWarnings(
        x <- getDesignGroupSequential(
            ...
            typeBetaSpending = "bsUser", userBetaSpending = c(0.15, 0.4, 0.8, 1) * 0.01, bindingFutility = FALSE
        )
    )

    ## Comparison of the results of TrialDesignGroupSequential object 'x' with expected results
    expect_equal(x$power, c(3.8817387e-06, 0.23459418, 0.9598886, 0.99), tolerance = 1e-07)
    expect_equal(x$futilityBounds, c(NA_real_, 0.33832475, 1.7778049), tolerance = 1e-07)
    ...
    expect_equal(y$shift, 23.351274, tolerance = 1e-07)
    expect_equal(y$inflationFactor, 1.1191952, tolerance = 1e-07)
    expect_equal(y$information, c(3.5026912, 9.3405098, 18.68102, 23.351274), tolerance = 1e-07)
    expect_equal(y$power, c(3.8817387e-06, 0.23459418, 0.9598886, 0.99), tolerance = 1e-07)
    expect_equal(y$rejectionProbabilities, c(3.8817387e-06, 0.2345903, 0.72529442, 0.030111396), tolerance = 1e-07)
    expect_equal(y$futilityProbabilities, c(0, 0.0029411764, 0.0047058824), tolerance = 1e-07)
    expect_equal(y$averageSampleNumber1, 0.79628245, tolerance = 1e-07)
...
test_that("'getDesignGroupSequential' with two-sided beta spending, kMax = 4, binding futility, KD beta spending, gammaB = 1.35", {
    .skipTestIfDisabled()
...
test_that("'getDesignGroupSequential' with two-sided beta spending, kMax = 4, binding futility, KD beta spending, gammaB = 1", {
    .skipTestIfDisabled()
...
test_that("'getDesignGroupSequential' with two-sided beta spending, kMax = 4, binding futility, user defined beta spending", {
    .skipTestIfDisabled()
    ...
    suppressWarnings(
        x <- getDesignGroupSequential(
            informationRates = c(0.35, 0.4, 0.8, 1),
            alpha = 0.025, typeOfDesign = "asOF", beta = 0.07, sided = 2,
            typeBetaSpending = "bsUser", userBetaSpending = c(0.15, 0.4, 0.8, 1) * 0.01, bindingFutility = TRUE
        )
    )

    ## Comparison of the results of TrialDesignGroupSequential object 'x' with expected results
    expect_equal(x$power, c(0.10679919, 0.21860669, 0.95487449, 0.99), tolerance = 1e-07)
    expect_equal(x$futilityBounds, c(NA_real_, 0.30649569, 1.7163392), tolerance = 1e-07)
    expect_equal(x$alphaSpent, c(4.8451862e-05, 0.00015681312, 0.010459859, 0.02499999), tolerance = 1e-07)
    expect_equal(x$betaSpent, c(0, 0.0029411765, 0.0076470589, 0.01), tolerance = 1e-07)
    ...
    if (isTRUE(.isCompleteUnitTestSetEnabled())) {
        ...
        expect_output(summary(x)$show())
        suppressWarnings(xCodeBased <- eval(parse(text = getObjectRCode(x, stringWrapParagraphWidth = NULL))))
        suppressWarnings(expect_equal(xCodeBased$power, x$power, tolerance = 1e-05))
        expect_equal(xCodeBased$futilityBounds, x$futilityBounds, tolerance = 1e-05)
        expect_equal(xCodeBased$alphaSpent, x$alphaSpent, tolerance = 1e-05)
        expect_equal(xCodeBased$betaSpent, x$betaSpent, tolerance = 1e-05)
        expect_equal(xCodeBased$criticalValues, x$criticalValues, tolerance = 1e-05)
    ...
    expect_equal(y$information, c(5.1671009, 5.9052581, 11.810516, 14.763145), tolerance = 1e-07)
    expect_equal(y$power, c(0.036739326, 0.087021046, 0.80817835, 0.93), tolerance = 1e-07)
    expect_equal(y$rejectionProbabilities, c(0.036739326, 0.05028172, 0.7211573, 0.12182165), tolerance = 1e-07)
    expect_equal(y$futilityProbabilities, c(0, 0.013748548, 0.0362213), tolerance = 1e-07)
    expect_equal(y$averageSampleNumber1, 0.84003166, tolerance = 1e-07)
    expect_equal(y$averageSampleNumber01, 0.86354683, tolerance = 1e-07)
    expect_equal(y$averageSampleNumber0, 0.76707978, tolerance = 1e-07)
    if (isTRUE(.isCompleteUnitTestSetEnabled())) {
        invisible(capture.output(expect_error(print(y), NA)))
        expect_output(print(y)$show())
        invisible(capture.output(expect_error(summary(y), NA)))
...
test_that("'getDesignGroupSequential' with two-sided beta spending, kMax = 4, non-binding futility, KD beta spending, gammaB = 1", {
    .skipTestIfDisabled()
...
test_that("'getDesignGroupSequential' with two-sided beta spending, kMax = 4, non-binding futility, KD beta spending, gammaB = 1.35", {
    .skipTestIfDisabled()
