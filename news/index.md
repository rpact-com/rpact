...
- Extended
  ...
  with additional options for report customization and metadata
  handling. A new argument `reportFileBaseName` allows explicit control
  over the base name of all generated report files.
- [`testPackage()`](https://docs.rpact.org/reference/testPackage.md) now
  supports structured metadata via the new arguments `metaData` and
  `metaDataFile`. Users can either pass a named list of key–value pairs
  directly or provide an external text file in `KEY=VALUE` format. This
  metadata is included in the generated test reports and supports
  reproducible, auditable installation qualification and validation
  workflows.
- Added two internal utility functions,
  ...
  for writing and reading human-editable key–value metadata files
  (`KEY=VALUE`, INI/.env-like format). These functions are implemented
  using base R only, support UTF-8 encoding, comments, and robust
  parsing, and are intended for use in regulated environments.
- The new key–value file utilities are used internally by
  [`testPackage()`](https://docs.rpact.org/reference/testPackage.md) to
  support structured metadata handling via the `metaData` and
  `metaDataFile` arguments, enabling reproducible and auditable
  inclusion of environment- and system-specific information in
  installation qualification and validation reports.
...
- Adjusted the calculation in
  [`getHazardRatioByPi()`](https://docs.rpact.org/reference/utilitiesForSurvivalTrials.md)
  to return the hazard ratio directly derived from the corresponding
  rate parameters, i.e., the hazard ratio is defined as `lambda1` /
  `lambda2`; the Weibull shape parameter `kappa` controls
...
- Corrected calculation of `finalPValues` for two-sided group-sequential
  designs in specific edge cases (see issue
  [\#114](https://github.com/rpact-com/rpact/issues/114)).
...
- [`setupPackageTests()`](https://docs.rpact.org/reference/setupPackageTests.md)
...
- [`resetOptions()`](https://docs.rpact.org/reference/resetOptions.md)
...
- Line breaks in the output of
  [`getObjectRCode()`](https://docs.rpact.org/reference/getObjectRCode.md)
...
- The
  [`fetch()`](https://docs.rpact.org/reference/fetch.ParameterSet.md)
  and
  [`obtain()`](https://docs.rpact.org/reference/fetch.ParameterSet.md)
...
- Analysis progress messages are only displayed when R is used
  interactively
- Manual use of
  [`kable()`](https://docs.rpact.org/reference/kableParameterSet.md) for
  rpact result objects marked as deprecated, as the formatting and
...
- [`getObjectRCode()`](https://docs.rpact.org/reference/getObjectRCode.md)
  (short:
  [`rcmd()`](https://docs.rpact.org/reference/getObjectRCode.md)): with
  the new arguments `pipeOperator` and `output` many new output variants
  can be specified, e.g., the native R pipe operator or the magrittr
...
- Design objects can be piped into
  [`getDataset()`](https://docs.rpact.org/reference/getDataset.md) to
  enable pipe syntax for analysis, e.g.,
...
- [`getRawData()`](https://docs.rpact.org/reference/getRawData.md): the
  resulting `data.frame` now contains the correct `stopStage` and
...
- Function
  [`testPackage()`](https://docs.rpact.org/reference/testPackage.md): a
  problem with downloading full set of unit tests under Debian/Linux has
  been fixed
- Generic function
  [`kable()`](https://docs.rpact.org/reference/kableParameterSet.md)
...
- The new function
  [`getObjectRCode()`](https://docs.rpact.org/reference/getObjectRCode.md)
  (short:
  [`rcmd()`](https://docs.rpact.org/reference/getObjectRCode.md))
...
- Issue in
  [`getAvailablePlotTypes()`](https://docs.rpact.org/reference/getAvailablePlotTypes.md)
...
- [`getRawData()`](https://docs.rpact.org/reference/getRawData.md):
...
- [`getStageResults()`](https://docs.rpact.org/reference/getStageResults.md):
  ...
  was corrected
- [`getDataset()`](https://docs.rpact.org/reference/getDataset.md) of
  survival data: issue with the internal storage of log ranks fixed
