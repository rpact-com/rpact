...
plot.EventProbabilities <- function(x, y, ..., 
	...
	if (!is.na(yObjectName)) {
		srcCmd2 <- .showPlotSourceInformation(objectName = yObjectName, 
			xParameterName = "time", 
...
plot.NumberOfSubjects <- function(x, y, ..., 
	...
	if (!missing(y) && inherits(y, "EventProbabilities")) {
		return(plot.EventProbabilities(x = y, y = x, 
	...
	if (!is.na(allocationRatioPlanned)) {
		warning("'allocationRatioPlanned' (", allocationRatioPlanned, 
			") will be ignored because 'y' is undefined (for more information see ?plot.NumberOfSubjects)", call. = FALSE)
	}
	...
	srcCmd <- .showPlotSourceInformation(objectName = objectName, 
		xParameterName = "time", 
		yParameterNames = "numberOfSubjects", 
		type = type,
		showSource = showSource)
