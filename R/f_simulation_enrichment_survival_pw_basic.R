...
#' @noRd
#'
.getSimulatedStageResultsSurvivalEnrichmentPatientWise <- function(
        ...,
        design,
        weights,
        subGroups,
        ...
        if (k < kMax) {
            ...
            if (adaptations[k]) {
                ...
                newEvents <- calcEventsFunction(
                    ...
                    plannedEvents = plannedEvents,
                    eventsOverStages = colSums(populationEventsPerStage, na.rm = TRUE),
...
.getSimulationEnrichmentSurvivalPatientWiseBasic <- function(
    ...
    for (i in seq_len(cols)) {
        for (j in seq_len(maxNumberOfIterations)) {
            stageResults <- .getSimulatedStageResultsSurvivalEnrichmentPatientWise(
                design = design,
                weights = weights,
