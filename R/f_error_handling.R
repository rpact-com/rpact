...
.createCondition <- function(
    ...
    if (is.function(factory)) {
        condition <- tryCatch(
            {
                # Omit absent optional fields for compatibility with existing factories.
                # Quote language-valued arguments so diagnostic calls are not evaluated.
                factoryArguments <- list(
